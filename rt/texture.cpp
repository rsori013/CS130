...
// vec3 Texture::Get_Color(const vec2& uv) const
// {
//     // Convert the UVs into pixel-space, but keep them as floating-point for now
//     float u_pixelSpace = uv[0] * width;
//     float v_pixelSpace = uv[1] * height;

//     // Wrap the UVs in pixel space and then convert to integer indices
//     int u_index = wrap(static_cast<int>(u_pixelSpace), width);
//     int v_index = wrap(static_cast<int>(v_pixelSpace), height);

//     int pixel_index = v_index * width + u_index;

//     // Debugging outputs
//     std::cout << "UV Coordinates: (" << uv[0] << ", " << uv[1] << ")" << std::endl;
//     std::cout << "Pixel Space UVs: (" << u_pixelSpace << ", " << v_pixelSpace << ")" << std::endl;
//     std::cout << "Pixel Indices: u = " << u_index << ", v = " << v_index << std::endl;

//     if (pixel_index >= 0 && pixel_index < width * height)
//     {
//         vec3 color = From_Pixel(data[pixel_index]);
//         std::cout << "Extracted Color: (" << color[0] << ", " << color[1] << ", " << color[2] << ")" << std::endl;
//         return color;
//     }
...
//     // Using floor function for rounding to ensure proper rounding
...
//     std::cout << "UV Coordinates: (" << uv[0] << ", " << uv[1] << ")" << std::endl;
//     std::cout << "Pixel Space UVs: (" << u_pixelSpace << ", " << v_pixelSpace << ")" << std::endl;
//     std::cout << "Pixel Indices (after rounding and wrapping): u = " << u_index << ", v = " << v_index << std::endl;
...
vec3 Texture::Get_Color(const vec2& uv) const
{
    ...
    // Flip the V coordinate. This might be needed if the texture appears upside-down.
    float v_pixelSpace = (1.0 - uv[1]) * height;
